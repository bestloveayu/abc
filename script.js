const steps = [
    ...
    async function submitToGoogleForm() {
        ...
        try {
            ...
            if (result.success) {
                ...
                console.error("提交資料失敗:", result.error);
            }
        } catch (error) {
    ...
    async function predictCocktail() {
        ...
        console.log("檢查 tmImage 是否已定義（predictCocktail）...");
        const tmImageLoaded = await waitForTmImage();
        if (!tmImageLoaded) {
            teachableResult.innerText = "錯誤：tmImage 未定義，請確認 teachablemachine-image.min.js 是否正確載入";
            return;
        ...
        if (!model) {
            console.error("Teachable Machine 模型未載入，請先載入模型");
            teachableResult.innerText = "錯誤：Teachable Machine 模型未載入，嘗試重新載入...";
        ...
        try {
            ...
            teachableResult.innerText = `無法開啟攝影機：${error.message}。請確認攝影機權限並使用本地伺服器運行（http://localhost:3000）。`;
        }
    }
